...
public class ParabolaController : MonoBehaviour
{
    ...
    public void FollowParabola()
    {
        ...
        RefreshTransforms(Speed);
        animationTime = 0f; // IMPORTANT TO MOVE!
        transform.position = parabolaFly.Points[0].position;
        Animation = true; // IMPORTANT TO MOVE!
    ...
    public class ParabolaFly
    {
        ...
        public Transform[] Points; // Actual point of ParabolaRoot Object. Points don't have to me with specific names!. Murad Shahsuvarov
        ...
        public ParabolaFly(Transform ParabolaRoot)
        {
            ...
            // Sort points algorithm
            Transform[] tempPoints = transforms.ToArray();
            // Printing out unsorted points
            for (int i = 0; i < tempPoints.Length; i++)
            {
                Debug.Log("BEFORE( " + i + " point: " + tempPoints[i] + " )");
            }
            Dictionary<int, string> pointsIndeces = new Dictionary<int, string>();
            pointsIndeces.Add(0,"StartPoint");
            pointsIndeces.Add(1,"MiddlePoint");
            pointsIndeces.Add(2,"EndPoint");

            // 0: EndPoint, 1: StartPoint, 2: MiddlePoint
            for (int i = 0; i < tempPoints.Length; i++)
            {
                if (tempPoints[i].tag != pointsIndeces[i])
                {
                    int key = pointsIndeces.FirstOrDefault(x => x.Value == tempPoints[i].tag).Key; // key of unplaced object
                    Transform temp = tempPoints[i];
                    tempPoints[i] = tempPoints[key];
                    tempPoints[key] = temp;

                }
            }
            Points = tempPoints;
            // Printing out sorted points
            for (int i = 0; i < tempPoints.Length; i++)
            {
                Debug.Log("AFTER( " + i + " point: " + tempPoints[i] + " )");
            }

            // Prints what point is what
            Debug.Log($"start point: {Points[0].tag}");
            Debug.Log($"middle point: {Points[1].tag}");
            Debug.Log($"end point: {Points[2].tag}");

            //check if odd
            if ((Points.Length - 1) % 2 != 0)
            {
                Debug.Log("ParabolaRoot needs odd number of points");
